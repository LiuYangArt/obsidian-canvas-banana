...
# Obsidian Plugin Audit Checklist (Updated 2025-12-17)

基于 Obsidian Review Bot 的反馈更新，严格遵循以提交 PR 审核。

> [!IMPORTANT]
> 提交前必须运行 `npm run lint`，确保安装了 `eslint-plugin-obsidianmd`。
> Review Bot 会检测代码中的 `eslint-disable` 注释，某些规则**禁止被禁用**。
...
**重要**: 对于语言包文件 (`lang/locale/en.ts`)，`obsidianmd/ui/sentence-case-locale-module` 规则**不允许使用 `eslint-disable` 注释**。

如需处理品牌名、技术占位符、URL 等特殊情况，必须在 ESLint 配置中使用 `brands`、`ignoreWords`、`ignoreRegex` 选项：

```javascript
// eslint.config.mjs 中的语言包规则配置
"obsidianmd/ui/sentence-case-locale-module": ["error", {
    brands: ["Gemini", "Google", "OpenRouter", "API", "URL", "JSON"],
    ignoreWords: ["sk", "v1", "AIza"],
...
export default [
    // 针对英文语言文件的 Sentence Case 检查
    {
        files: ["**/en.ts", "**/en-*.ts"],
        plugins: { "obsidianmd": obsidianmd },
        rules: {
            "obsidianmd/ui/sentence-case-locale-module": ["error", {
                brands: ["Gemini", "Google", "API", "URL"],
...
### 23. 为什么本地 lint 没检测出问题？

常见原因：
1. **规则等级不同**: 本地设置为 `warn`，但 Review Bot 使用 `error`。建议本地也设为 `error`。
2. **禁用注释被忽略**: 本地 ESLint 尊重 `eslint-disable` 注释，但 Review Bot 会检测并报错（某些规则禁止被禁用）。
3. **配置不完整**: 未正确配置 `sentence-case-locale-module` 规则的白名单选项。
...
- [ ] `npm run build` 构建成功
- [ ] `npm run lint` 无错误（ESLint 包含 eslint-plugin-obsidianmd）
- [ ] 所有 UI 文本使用 Sentence case（仅首字母大写）
- [ ] **语言包文件无 `eslint-disable` 注释**（使用配置白名单代替）
