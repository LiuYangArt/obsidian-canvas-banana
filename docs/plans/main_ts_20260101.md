...
1. **Phase 1: 工具函数** - `image-viewer.ts`, `node-operations.ts`
2. **Phase 2: Ghost Node** - `ghost-node-manager.ts` (中风险)
3. **Phase 3: Selection** - `selection-listener.ts` (高风险)
4. **Phase 4: Generation** - `generation-handler.ts` (最高风险)
